...
/*
 * This class has been modified to eliminate the static members which
 * prevent multiple instances of the class being used simultaneously
 * without each instance polluting the other's data.
 *
 *  -- Peter Keeler, 2017
 */
public class SimplexNoise implements Noise2D, Noise3D { // Simplex noise in
    // 2D, 3D and 4D
    private long seed = 0;
    ...
    private static final double dot(Grad g, double x, double y) {
        return g.x * x + g.y * y;
    }

    private static final double dot(Grad g, double x, double y, double z) {
        return g.x * x + g.y * y + g.z * z;
    }

    private static final double dot(Grad g, double x, double y, double z, double w) {
